...
# Activity indicator ranges for rot_amp
...
echo "Activity indicator rot_amp ranges:"
...
EOF
    ...
    # Add GP model (always present now since white_noise is removed)
    cat >> "$yaml_file" << EOF
      - gp_multidimensional
EOF
...
EOF
    ...
    # Add activity section (always present now)
    cat >> "$yaml_file" << EOF
  activity:
    boundaries:
      Prot: [20.0, 35.0]
...
EOF
    ...
    # Add GP multidimensional model (always present now)
    cat >> "$yaml_file" << EOF
  gp_multidimensional:
...
EOF
    ...
    for indicator in "${indicators[@]}"; do
        ...
        rot_amp_range="${indicator_ranges[$indicator]}"
        
        cat >> "$yaml_file" << EOF
    ${indicator}data:
      boundaries:
        rot_amp: [${rot_amp_range}]
        con_amp: [-10.0, 10.0]
      derivative: True
EOF
    done
...
job_ids=$(bjobs | grep -E "(DS[1-9]_[0-3]p_(2_activity_indi|4_activity_indi|5_activity_indi|ccfs)_single)" | awk '{print $1}')
...
echo "  - CaII: [-1, 1]"
echo ""
echo "Eccentricity prior: Beta(1.12, 3.09) - Kipping 2013"
echo ""
echo "Configuration details:"
echo "- All configurations use GP multidimensional model"
echo "- 0p: 0 planets with GP activity model only (4 configurations)"
echo "- 1p-3p: 1-3 planets with GP activity model (4 configurations each)"
echo "- 2_activity_indi: BIS, FWHM"
echo "- 4_activity_indi: BIS, FWHM, Contrast, Halpha"
echo "- 5_activity_indi: BIS, FWHM, Contrast, Halpha, CaII"
echo "- ccfs: FWHM, Contrast"
...
echo ""
echo "Total jobs: 144 (9 datasets × 4 planets × 4 configurations)"
echo "  - Per dataset: 16 jobs (4 planets × 4 configurations)"
