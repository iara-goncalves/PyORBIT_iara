...
# PyORBIT Complete Setup Generator - Single Files Version
# Creates directory structure, YAML configs, and job scripts
# Structure: ESSP4/scripts/ (this script) and ESSP4/results_single/ (results)
# Data files: ESSP4/data/ (single combined .dat files, not per instrument)
...
results_dir="/work2/lbuc/iara/GitHub/PyORBIT_examples/ESSP4/results_single"
out_dir="/work2/lbuc/iara/GitHub/PyORBIT_examples/ESSP4/out_single"
...
echo "Data: Single combined files (not per instrument)"
echo "Configurations:"
...
EOF
    ...
    # Add GP model only if we have activity indicators (not for white_noise)
...
EOF
    ...
    # Add activity indicator inputs (if any)
    if [ ${#indicators[@]} -gt 0 ]; then
        for indicator in "${indicators[@]}"; do
            cat >> "$yaml_file" << EOF
  ${indicator}data:
    file: ${data_dir}/${dataset}_${indicator}.dat
    kind: ${indicator}
    models:
      - gp_multidimensional
EOF
...
for dataset in "${datasets[@]}"; do
    # Create main dataset directory in results_single
    ...
    for planet in "${planets[@]}"; do
        ...
        for config_name in "${!config_indicators[@]}"; do
            ...
            # Skip white_noise for 0p (0 planets with no activity doesn't make sense)
            ...
            job_name="${dataset}_${planet}_${config_name}_single"
...
# Submit all jobs
cat > "submit_all_jobs_single.sh" << 'EOF'
#!/bin/bash
...
# Submit 0p jobs only
cat > "submit_0p_jobs_single.sh" << 'EOF'
#!/bin/bash
...
cat > "submit_2_activity_indi_single.sh" << 'EOF'
#!/bin/bash

echo "Submitting 2_activity_indi jobs (single files, BIS, FWHM)..."
echo "============================================================"

job_count=0
for script in run_*_2_activity_indi_single.sh; do
    if [ -f "$script" ]; then
        echo "Submitting: $script"
        bsub < "$script"
        ((job_count++))
...
echo "Submitted $job_count 2_activity_indi single file jobs"
EOF

# Submit 4_activity_indi jobs only
cat > "submit_4_activity_indi_single.sh" << 'EOF'
#!/bin/bash
...
cat > "submit_5_activity_indi_single.sh" << 'EOF'
#!/bin/bash

echo "Submitting 5_activity_indi jobs (single files, BIS, FWHM, Contrast, Halpha, CaII)..."
echo "===================================================================================="

job_count=0
for script in run_*_5_activity_indi_single.sh; do
    if [ -f "$script" ]; then
        echo "Submitting: $script"
        bsub < "$script"
        ((job_count++))
...
echo "Submitted $job_count 5_activity_indi single file jobs"
EOF

# Submit ccfs jobs only
cat > "submit_ccfs_single.sh" << 'EOF'
#!/bin/bash
...
# Submit white_noise jobs only
cat > "submit_white_noise_single.sh" << 'EOF'
#!/bin/bash
...
# Monitor jobs
cat > "monitor_jobs_single.sh" << 'EOF'
#!/bin/bash
...
# Cancel all jobs
cat > "cancel_all_jobs_single.sh" << 'EOF'
#!/bin/bash

echo "Canceling all PyORBIT single file jobs (all configurations)..."
echo "=============================================================="

job_ids=$(bjobs | grep -E "(DS[1-9]_[0-3]p_(2_activity_indi|4_activity_indi|5_activity_indi|ccfs|white_noise)_single)" | awk '{print $1}')

if [ -z "$job_ids" ]; then
    echo "No PyORBIT single file jobs found to cancel."
    exit 0
fi

echo "Found PyORBIT single file jobs to cancel:"
echo "$job_ids"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    echo "All PyORBIT single file jobs canceled."
else
    echo "Operation canceled."
fi
EOF

chmod +x submit_all_jobs_single.sh
chmod +x submit_0p_jobs_single.sh
chmod +x submit_2_activity_indi_single.sh
chmod +x submit_4_activity_indi_single.sh
chmod +x submit_5_activity_indi_single.sh
chmod +x submit_ccfs_single.sh
chmod +x submit_white_noise_single.sh
chmod +x monitor_jobs_single.sh
chmod +x cancel_all_jobs_single.sh
...
echo "    │   └── DS5_1p_white_noise/         (1 planet + RV only)"
echo "    ├── DS5_2p/ (same 5 configurations)"
echo "    └── DS5_3p/ (same 5 configurations)"
echo ""
echo "Data files used (single combined files):"
echo "- RV data: DS5_RV.dat"
echo "- Activity indicators: DS5_BIS.dat, DS5_FWHM.dat, DS5_Contrast.dat, DS5_Halpha.dat, DS5_CaII.dat"
echo ""
echo "Configuration details:"
echo "- 0p: 0 planets with GP activity model only (4 configurations)"
echo "- 1p-3p: 1-3 planets with optional GP activity model (5 configurations each)"
echo "- 2_activity_indi: BIS, FWHM"
echo "- 4_activity_indi: BIS, FWHM, Contrast, Halpha"
echo "- 5_activity_indi: BIS, FWHM, Contrast, Halpha, CaII"
echo "- ccfs: FWHM, Contrast"
echo "- white_noise: RV only (not available for 0p)"
...
echo "- ./submit_all_jobs_single.sh           - Submit ALL single file configurations"
echo "- ./submit_0p_jobs_single.sh            - Submit 0 planets configurations only"
echo "- ./submit_2_activity_indi_single.sh    - Submit 2 activity indicators only"
echo "- ./submit_4_activity_indi_single.sh    - Submit 4 activity indicators only"
echo "- ./submit_5_activity_indi_single.sh    - Submit 5 activity indicators only"
echo "- ./submit_ccfs_single.sh               - Submit ccfs only"
echo "- ./submit_white_noise_single.sh        - Submit white noise only"
echo "- ./monitor_jobs_single.sh              - Monitor all single file jobs"
echo "- ./cancel_all_jobs_single.sh           - Cancel all single file jobs"
echo ""
echo "Total jobs: 171 (9 datasets × [4 configs for 0p + 5 configs × 3 planets])"
echo "  - 0p jobs: 36 (9 datasets × 4 configurations)"
echo "  - 1p-3p jobs: 135 (9 datasets × 3 planets × 5 configurations)"
