    ...
    try:
        with fits.open(fits_file) as hdul:
            ...
            data = {
                ...
                'instrument_flag': 2 if bjd < HARPS_CORRECTION_BJD else 3  # HARPS pre-correction = 2, post-correction = 3
            }
    ...
    for indicator, info in indicators.items():
        ...
        if len(subset_df) > 0:
            ...
            if instrument_name == 'HARPS':
                ...
                pre_correction = len(subset_df[subset_df['instrument_flag'] == 2])
                post_correction = len(subset_df[subset_df['instrument_flag'] == 3])
                print(f"  - Pre-correction (flag=2): {pre_correction}, Post-correction (flag=3): {post_correction}")
        else:
...
def save_combined_dat_files(df, output_dir, exclude_outliers=True):
    ...
    for indicator, info in indicators.items():
        ...
        if len(combined_df) > 0:
            ...
            flag_names = {1: 'ESPRESSO', 2: 'HARPS pre-correction', 3: 'HARPS post-correction'}
    ...
    for i, indicator in enumerate(indicators):
        ...
        for instrument in ['ESPRESSO', 'HARPS']:
            ...
            if mask.sum() > 0:
                ...
                if instrument == 'HARPS':
                    # Pre-correction HARPS (flag = 2)
                    pre_mask = mask & (df['instrument_flag'] == 2)
                    if pre_mask.sum() > 0:
                    ...
                    # Post-correction HARPS (flag = 3)
                    post_mask = mask & (df['instrument_flag'] == 3)
                    if post_mask.sum() > 0:
...
def main():
    ...
    if len(combined_df) > 0:
        ...
        print(f"Instrument flags: 1=ESPRESSO, 2=HARPS pre-correction, 3=HARPS post-correction")
